...
每种类型是一组值。前五个组都是有限的。当然，只有两个布尔值，真与假，而且他们不会制造新的。但是有更多的数值和字符串值。该标准说，其有18,437,736,874,454,810,627个不同的数字(包括NaN，则其名字为“不是一个数”的缩写)。这与可能的字串的数量是不能相比的。我认为字符串的数目可以是（2144,115,188,075,855,872 - 1）÷65,535...虽然我可能计算有误。
