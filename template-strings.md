...
通过这个定义，标签模版 SaferHTML\``<p>${bonk.sender} has sent you a bonk.</p>`\` 可能扩展成字符串 `"<p>ES6&lt;3er has sent you a bonk.</p>"`。你的用户是安全的，即使一个恶意命名的用户，比如 `Hacker Steve <script>alert('xss');</script>`，就给他们一个撞击。不用管他们到死是什么意思。
