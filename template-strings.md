...
- 在一个模板替换中的代码可以是任何 JavaScript 表达式，所以函数调用，算术等操作都是允许的。(如果你真的愿意，你甚至可以嵌套一个模版字符串于另一个模板字符串中，我称之为模板开始。）

- 如果任一值不是一个字符串，使用通常的规则，它可以被转换成字符串。例如，如果动作是一个对象，它的 .toString() 方法将被调用。

- 如果你需要在一个字符串模板内写反引号，你必须在其前面用反斜杠：\`\ \` \`与 " \`  "是一样的。

- 同样，如果你需要 在模板中的字符串中包括两个字符 \${，我不想知道你在做什么，但你可以用反斜杠来进行转义：\` write \\\${ or $\\{ \`。
...
好了，因为我上周的承诺，我对你们头脑的健康负有责任。所以，快速的给你们一个警告：从这里开始，事情将会变得有点紧张。你现在可以停止阅读了，你可以先去喝杯咖啡并享受下你的未受损害且还没变糊涂的脑袋。严重的是，走回头路是没有羞耻的。在证明了船舶能穿越赤道而不会被海怪破坏，或是从地球的边缘掉下去后，Lopes Gonçalves 彻底探索了整个南半球了吗？不，他回头了。他回家了，并吃了一顿不错的午餐。你也很喜欢吃午饭吧？
...
- 他们不是模版库的替代，比如 Mustache 或者 Nunjucks。
...
我们看到，未标记的模板字符串只是简单字符串连接的简写。标签模板完全简写别的东西：函数调用。
...
```
通过这个定义，标签模版 SaferHTML\``<p>${bonk.sender} has sent you a bonk.</p>`\` 可能扩展成字符串
`"<p>ES6&lt;3er has sent you a bonk.</p>"`。你的用户是安全的，即使一个恶意命名的用户，比如 `Hacker Steve <script>alert('xss');</script>`，就给他们一个撞击。不用管他们到死是什么意思。
...
- 模板字符串没有内置的国际化功能。但通过使用标签，我们将他们添加进去。杰克许的一个博客帖子展示了这个方法的第一个步骤可能是什么样子的。仅举一例，作为预告：
<pre><code>
i18n\`Hello ${name}, you have ${amount}:c(CAD) in your bank account.`
// => Hallo Bob, Sie haben 1.234,56 $CA auf Ihrem Bankkonto.


	注意如何在这个例子中，名称和数量是 JavaScript 的，但不熟悉的代码有一个不同的位，即:c(CAD)，在模板的字符串部分杰克所放的地方。JavaScript 是理所当然由 JavaScript 引擎处理;字符串部分由杰克的 i18n 标签处理。用户将从国际化文档中学习到，:c(CAD) 指的是 amount 是用加元结算的货币的量。
...
在浏览器中，火狐浏览器34以上的版本是支持模版字符串的。他们是由 Guptha Rajagopal 在上个夏天作为一个实习项目完成的。谷歌浏览器41以上的版本也支持模版字符串，但是微软浏览器 IE 和苹果的浏览器 Safari 不支持。到现在为止，如果你想在网络上使用模版字符串，你将需要使用Babel 或者 Traceur。你也可以在 TypeScript 中立即进行使用。
...
这带来了点问题！如果你在 Markdown 文档中写这个字符：
...
Gist 是有细节的，且其是在 Markdown 中编写，所以你可以看看源。

## 下一个
下周，我们将会看看其他语言中被程序员已经享用了几十年的两个功能：一个是提供给那些当可以避免使用参数就不使用参数的人，一个是提供给那些喜欢使用很多参数的人。我当然会讨论有关函数参数的东西。这两种功能是真正为我们所有的。
